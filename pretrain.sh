...
accelerate launch --num_processes $(( 4 * $NUM_HOSTS )) --num_machines $NUM_HOSTS --multi_gpu --mixed_precision bf16 --machine_rank 0 --main_process_ip $MASTER_ADDR --main_process_port $MASTER_PORT --gradient_accumulation_steps 5 pretrain.py --config configs/pretrain.yaml
