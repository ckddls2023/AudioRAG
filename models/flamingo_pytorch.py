...
class PerceiverResampler(nn.Module):
    def __init__(self, dim, depth, dim_head=64, heads=8,num_latents=64,num_media_embeds=4,ff_mult=4,grad_checkpoint=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for attn, ff in self.layers:
            if self.grad_checkpoint:
                latents = torch.utils.checkpoint.checkpoint(attn, x, latents) + latents
                latents = torch.utils.checkpoint.checkpoint(ff, latents) + latents
            else:
    ...
    def forward(
        ...
        if exists(media_locations):
            text_time = media_locations.cumsum(
                dim=-1)  # at each boolean of True, increment the time counter (relative to media time)
            ...
            text_to_media_mask = mask_op(rearrange(text_time, 'b i -> b 1 i 1'),
